...
$(function() {
	                	...
	                	"render": function ( data, type, row ) {
	                		return function(){
                                ...
                                if (1 == row.triggerStatus ) {
                                    start_stop_div = '<li><a href="javascript:void(0);" class="job_operate" _type="job_pause" >'+ I18n.jobprocess_opt_stop +'</a></li>\n';
                                } else {
                                    start_stop_div = '<li><a href="javascript:void(0);" class="job_operate" _type="job_resume" >'+ I18n.jobprocess_opt_start +'</a></li>\n';
                                }

                                // log url
                                var logHref = base_url +'/joblog?jobId='+ row.pid;

                                // log url
                                var codeBtn = "";
                                if ('BEAN' != row.glueType) {
                                    var codeUrl = base_url +'/jobcode?jobId='+ row.pid;
                                    codeBtn = '<li><a href="'+ codeUrl +'" target="_blank" >GLUE IDE</a></li>\n';
                                }

                                // data
                                tableData['key'+row.pid] = row;
                                ...
                                var html = '<div class="btn-group">\n' +
                                    ...
                                    '     </button>\n' +
                                    '     <ul class="dropdown-menu" role="menu" _id="'+ row.pid +'" >\n' +
                                    '       <li><a href="javascript:void(0);" class="job_trigger" >'+ I18n.jobprocess_opt_run +'</a></li>\n' +
                                    '       <li><a href="'+ logHref +'">'+ I18n.jobprocess_opt_log +'</a></li>\n' +
                                    '       <li><a href="javascript:void(0);" class="job_registryinfo" >' + I18n.jobprocess_opt_registryinfo + '</a></li>\n' +
                                    '       <li class="divider"></li>\n' +
                                    start_stop_div +
                                    codeBtn +
	...
	$('#jobGroup').on('change', function(){
        ...
        window.location.href = base_url + "/jobprocess?jobGroup=" + jobGroup;
    });
	...
	$("#job_list").on('click', '.job_operate',function() {
		...
		if ("job_pause" == type) {
			typeName = I18n.jobprocess_opt_stop ;
			url = base_url + "/jobprocess/stop";
			needFresh = true;
		} else if ("job_resume" == type) {
			typeName = I18n.jobprocess_opt_start ;
			url = base_url + "/jobprocess/start";
			needFresh = true;
		} else if ("job_del" == type) {
			typeName = I18n.system_opt_del ;
			url = base_url + "/jobprocess/remove";
			needFresh = true;
		} else {
			return;
		}
    ...
    $("#jobTriggerModal .ok").on('click',function() {
        $.ajax({
            type : 'POST',
            url : base_url + "/jobprocess/trigger",
            data : {
            ...
            success : function(data){
                if (data.code == 200) {
                    ...
                    layer.msg( I18n.jobprocess_opt_run + I18n.system_success );
                } else {
                    layer.msg( data.msg || I18n.jobprocess_opt_run + I18n.system_fail );
                }
            }
        });
    });
    ...
    $("#job_list").on('click', '.job_registryinfo',function() {
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                layer.open({
                    title: I18n.jobprocess_opt_registryinfo ,
                    btn: [ I18n.system_ok ],
                    content: html
                });
	...
	$(".add").click(function(){
        ...
        window.open(base_url + '/jobprocess/jobEditPage?operFlag=1');
        return;
	...
	var addModalValidate = $("#addModal .form").validate({
        ...
        messages : {  
            jobDesc : {
            	required : I18n.system_please_input + I18n.jobprocess_field_jobdesc
            },
            jobCron : {
            	required : I18n.system_please_input + "Cron"
            },
            author : {
            	required : I18n.system_please_input + I18n.jobprocess_field_author
            },
            executorTimeout : {
        ...
        submitHandler : function(form) {
        	...
        	$.post(base_url + "/jobprocess/add",  $("#addModal .form").serialize(), function(data, status) {
    			if (data.code == "200") {
					$('#addModal').modal('hide');
					layer.open({
						title: I18n.system_tips ,
	...
	$("#job_list").on('click', '.update',function() {
        ...
        var pid = $(this).parents('ul').attr("_id");
        window.open(base_url + '/jobprocess/jobEditPage?operFlag=2&pid='+pid);
        // var row = tableData['key'+id];
        //
		// // base data
	...
	var updateModalValidate = $("#updateModal .form").validate({
        ...
        submitHandler : function(form) {
			...
			// post
    		$.post(base_url + "/jobprocess/update", $("#updateModal .form").serialize(), function(data, status) {
    			if (data.code == "200") {
